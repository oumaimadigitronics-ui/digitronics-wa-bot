// server.js (Fixed-offers DigiBot + last-6 memory + size-only follow-up merge)
//
// What it does:
// - Receives WANotifier webhook at POST /wanotifier
// - Keeps last 6 client messages per WhatsApp number (in-memory, 24h TTL)
// - If user sends ONLY a TV size like "43 inch", it auto-merges with last known brand
//   and sends "bghit VISIO 43 inch" to OpenAI (more reliable than pure context)
// - Uses ONLY the fixed-offers system prompt (no WooCommerce)
// - Blocks audio/media (asks for text)
...
// Simple per-number rate limit store
...
// ===== Conversation memory (last 6 client messages per WA number) =====
...
// Cleanup old memory every 10 min
setInterval(() => {
  const now = Date.now();
...
// ===== Helpers: detect size-only + last brand from history =====
function extractSizeOnly(text) {
  const s = String(text || "").toLowerCase().trim();
  // Accept: "43", "43 inch", "43 inches", "43 pouce", '43"'
...
function extractBrandFromText(text) {
  ...
  // Only fixed-offers brands (TV context)
  ...
  if (s.includes("krohler")) return "TRIO_KROHLER";
  if (s.includes("trio")) return "TRIO_KROHLER";
...
// Call OpenAI with the fixed-offers prompt + last 6 user messages
...
app.post("/wanotifier", async (req, res) => {
  ...
  try {
    ...
    // Store message, get last 6
    const last6 = pushClientMessage(waNumber, userText);

    // If user wrote only a size like "43 inch", merge with last brand from history
    ...
    // If we changed it, push again so history contains the explicit intent
