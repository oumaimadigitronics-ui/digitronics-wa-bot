...
const BRAND_PRIORITY_FOR_PROMPT = [
  "TCL",
  "DAIKO",
  "HAIER",
  "SAMSUNG",
...
function trimOffersForPrompt(arr, opts) {
  ...
  const list = (Array.isArray(arr) ? arr : []).filter((x) => Number((x && x.stock) || 0) > 0);
  if (!hasTargetSize) {
    return list.slice(0, MAX_OFFERS_FOR_PROMPT);
  }

  const scored = list
    .map((x, idx) => {
      const sizeNum = Number((x && x.size) || 0);
      const sizeScore = Number.isFinite(sizeNum) ? Math.abs(sizeNum - targetSize) : Number.MAX_SAFE_INTEGER;
      return { offer: x, sizeScore, idx };
    })
    .sort((a, b) => {
...
function brandPriorityIndex(brand) {
  const idx = BRAND_PRIORITY_FOR_PROMPT.indexOf(String(brand || "").toUpperCase());
  return idx >= 0 ? idx : null;
}

function sortBrandsForPrompt(offersObj) {
  const brands = Object.keys(offersObj || {});
  if (!brands.length) return brands;

  const originalIndex = new Map(brands.map((b, i) => [b, i]));
  const hasPriorityBrand = brands.some((b) => brandPriorityIndex(b) !== null);
  if (!hasPriorityBrand) return brands;
...
function limitOffersForPromptPayload(data) {
  ...
  const orderedBrands = sortBrandsForPrompt(offersObj);
...
function buildOffersSubsetForPrompt(userText, historyMsgs, key) {
  ...
  const offersObj = (OFFERS && OFFERS.offers) || {};
  const orderedBrands = sortBrandsForPrompt(offersObj);
  const hasPriorityBrand = orderedBrands.some((b) => brandPriorityIndex(b) !== null);

  if (hasPriorityBrand) {
    const offers = {};
    for (const b of orderedBrands) {
      const arr = (offersObj[b] || []).filter((o) => Number((o && o.stock) || 0) > 0);
      if (arr.length) offers[b] = trimOffersForPrompt(arr, { size: sizeVal });
    }

    const capped = limitOffersForPromptPayload({ offers, meta: { size: sizeVal || null } });
    if (Object.keys(capped.offers || {}).length) return capped;
  }
